   ...
   <div class="d-flex flex-column flex-row-fluid gap-7 gap-lg-10">
      ...
      <div class="card card-flush py-4">
         ...
         <div class="card-body pt-0">
            ...
            @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
            <x-board.form.input type="text" name="title" :localeCode="$localeCode" :table="$contact ?? null" desc="{{ __('board.contact.title_desc') }}">{{ __("board.contact.title") }} {{ $properties['native'] }}</x-board.form.input>
            @endforeach

            @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
            <x-board.form.textarea name="body" :localeCode="$localeCode" :table="optional($contact ?? null)" desc="{{ __('board.contact.description_desc') }}">{{ __("board.contact.description") }} {{ $properties['native'] }}</x-board.form.textarea>
         @endforeach
